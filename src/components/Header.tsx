...
function Header() {
  ...
  return (
    <>
      <header
        className={`sg-nav${menuOpen ? " sg-nav--menu-open" : ""}`}
      >
      ...
      {/* Portaled into <body> so the drawer's position:fixed can't be
          captured by any ancestor's containing-block-forming property
          (transform, filter, backdrop-filter, contain, etc.). Combined
          with the opacity-only animation in nav.css, this keeps the
          drawer anchored to the viewport regardless of scroll position. */}
      {createPortal(
        <div
          ref={drawerRef}
          id="sg-mobile-menu"
